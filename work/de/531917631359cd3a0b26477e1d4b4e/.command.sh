...
set -euo pipefail
    ...
    if [ "$AZ_RC" -ne 0 ] || [ -z "$AZURE_SAS" ]; then
        echo "❌ Error: Failed to mint SAS token for container 'imaging-processed' on account '$STORAGE_ACCOUNT'."
...
AZURE_CONTAINER='imaging-processed'
AZURE_SAS='${AZURE_SAS}'
AZURE_SAS_EXPIRY='$EXPIRY'
AZURE_SAS_PERMISSIONS='cwrl'
